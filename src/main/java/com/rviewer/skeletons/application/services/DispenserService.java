...
public class DispenserService {
    ...
    public DispenserResponse create(DispenserRequest dispenserRequest) {
        ...
        return dispenserSaved
            .map(dispenser -> new DispenserResponse(dispenser.getId().getValue(), dispenser.getFlowVolume()))
    ...
    public void open(int id, LocalDateTime date) {
        ...
        dispenserById.ifPresentOrElse(dispenser -> { 
    ...
    public SpendingResponse getUsages(int id) {
        ...
        dispenserById.ifPresentOrElse(dispenser -> {
            ...
            if (dispenser.getStatus().isOpened()) {
                ...
                spending.getUsages().add(
                    new UsageResponse(
                        dispenserStatus.getOpenedAt(),
                        dispenserStatus.getOpenedAt().plusSeconds((long) dispenserStatus.getSecondsOpened()),
                        dispenser.getFlowVolume(),
                        dispenser.getLitersDispensed() * 12 // FIXME: add getAmount in domain aggregate
                    )
                );
            }
